 ...
 * Documentation (from email 2/26/2016):
   ...
   SUBROUTINE avec_timer_init ( taskid, retval )
    INTEGER, INTENT(IN) :: taskid     ! rank of MPI task on which this is called
...
Revision, 20210810  jm
   Changed from gettimeofday to clock_gettime in order to avoid
   using struct timezone, which is obsolete and generates an error
   when compiling with -std=c99: error: incomplete type is not allowed
...
#include <sys/time.h>
#include <omp.h>
#ifdef __xlf_compiler__
# define AVEC_MICROCLOCK avec_microclock
# define AVEC_MILLICLOCK avec_milliclock
# else
# define AVEC_MICROCLOCK avec_microclock_
# define AVEC_MILLICLOCK avec_milliclock_
#endif
...
// enable this to enable MPE traces along with AVEC timers. Must be linked to MPE library. Tested with mpe2-2.4.9b
//#define ENABLE_MPE

static int avec_timers_active = 0 ;
...
typedef struct zone {
  ...
  int maxintervals ;
#ifdef ENABLE_MPE
  int event_start ; // for MPE
  int event_stop ; // for MPE
#endif
...
#ifdef ENABLE_MPE
        if ( enable_mpe ) { char label[256] ;
          sprintf(label,"start %s",zones[zoneid].name ) ;
          MPE_Log_event( zones[zoneid].event_start, 0, label ) ; }
#endif
      }
      else {
...
#ifdef ENABLE_MPE
        if ( enable_mpe ) { char label[256] ;
          sprintf(label,"stop %s",zones[zoneid].name ) ;
          MPE_Log_event( zones[zoneid].event_stop, 0, label ) ; }
#endif
      }
      else {
...
{
#ifdef _OPENMP_
  if ( omp_get_thread_num() == 0 ) avec_timer_start_private( *zoneid, 0 ) ;
#else
...
{
#ifdef _OPENMP_
  if ( omp_get_thread_num() == 0 ) avec_timer_stop_private( *zoneid, 0 ) ;
#else
...
avec_timer_init_ ( int * taskid_p, int * retval )
{
...
#ifdef ENABLE_MPE
  char *col[] = { "red", "blue", "green", "orange" } ;
#endif
  ...
  *retval = 0 ;
  ...
  while ( fgets( inlne, 256, fp ) != NULL ) {
    ...
    if ( i == 1 )
    {
      if (!strcmp( tokens[0], "mpe" )) {
        enable_mpe = 1 ;
      }
      if (!strcmp( tokens[0], "nompe" )) {
        enable_mpe = 0 ;
      }
    }
#endif
...
#ifdef ENABLE_MPE
            if ( enable_mpe ) {
            zones[zoneid].event_start = MPE_Log_get_event_number() ;
            zones[zoneid].event_stop  = MPE_Log_get_event_number() ;
            if ( avec_timer_taskid == 0 ) {
               MPE_Describe_state( zones[zoneid].event_start, zones[zoneid].event_stop, zones[zoneid].name, col[zoneid % 4] ) ;
            }}
#endif
          }
        }
...
#ifdef ENABLE_MPE
    if ( enable_mpe ) {
      fprintf(stderr,"AVEC_TIMERS: ADVISORY: mpe tracing is enabled\n") ;
    } else {
      fprintf(stderr,"AVEC_TIMERS: ADVISORY: mpe tracing is not enabled\n") ;
    }
#endif
  }
...
void
avec_retrieve_( int * val )
{
  *val = stashedval  ;
}

void
avec_timers_fetch_( int * zoneid , int * result )
{
  int i, j ;
  for ( j = 0 ; j < MAXZONES ; j++ ) {
    if ( zones[j].zoneid == *zoneid ) {
      *result = 0 ;
      for ( i = 0 ; i < zones[j].zone_sequence_number ; i++ ) {
        *result += zones[j].intervals[i].stop_time-zones[j].intervals[i].start_time ;
      }
      return ;
    }
  }
...
#ifdef ENABLE_MPE
    if ( enable_mpe ) MPE_Finish_log("avec_timers") ;
#endif
  }
}
...
#if 0
static long int nnn = 0  ;
incrnnn_() { nnn++ ; }
shownnn_() { fprintf(stderr,"nnn = %ld\n",nnn) ; }
#endif
